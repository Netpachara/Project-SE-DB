...
$course_dept = "SELECT * FROM course INNER JOIN admin ON course.db_ID = admin.dept_ID INNER JOIN schduling_lec ON course.course_ID = schduling_lec.course_lecID";
$rlt = mysqli_query($connect,$admin_info);
$c_dept = mysqli_query($connect,$course_dept);
?>
    ...
    <div class="row" style = "top: 15%; position: absolute;">
        ...
        <div class="col-md-auto" style =  "text-align: left;">
            <ul class="nav nav-tabs" style="font-size:22px; font-weight:bold;">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="http://127.0.0.1/RegWeb/ModifyCourseAdmin.php">เเก้ไขกระบวนวิชา</a>
                </li>
                <li class="nav-item">
            ...
            <h5><label for='type' class = 'col-sm-3 control-label' style = "margin-top: 20;">Select semester & year</label></h5>
                ...
                <div style="margin-top: 30;">
                    <h1 style = "text-align: left; font-size:20px; margin-top: 15;">Select Course</h1>
                    <div class = 'from-group'>
                        <div class="clo-sm-12" style="margin-top: 20;">
                            <select name="courseID" class="form-contorl">
                                <option value="" selected="selected">- Select -</option>
                                <?php foreach($c_dept as $cd){ ?>
                                <option value= '<?=$cd['course_ID'],' ',$cd['section']?>'><?=$cd['course_ID'],' ',$cd['coursename'],' ',$cd['section']?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                ...
                <div class="mb-3">
                    <h1 style = "text-align: left; font-size:20px; margin-top: 15;">lecture room</h1>
                    <input style="width:300px;" type="text" class="form-control" id="lec_room" name = "lec_room">
                    <h1 style = "text-align: left; font-size:20px; margin-top: 15;">lab room</h1>
                    <input style="width:300px;" type="text" class="form-control" id="lab_room" name = "lab_room">
                    <?php 
                        $slottime = "SELECT * FROM slot_time";
                        $slot_time = mysqli_query($connect,$slottime);
                    ?>
                    <h1 style = "text-align: left; font-size:20px; margin-top: 15;">Select Slot time Study</h1>
                    <div class = 'from-group'>
                        <div class="clo-sm-12" style="margin-top: 20;">
                    ...
                    <h1 style = "text-align: left; font-size:20px; margin-top: 15;">จำนวนรับ</h1>
                    <input style="width:300px;" type="text" class="form-control" id="capacity" name = "capacity">
                    <button style ="background-color:grey; border-color:grey; margin-top: 20;" type="search" class="btn btn-primary" onclick = "alert('Modify success')" >modify</button>
                </div>
            </form>
        </div>
    </div>
