...
$student_email = "SELECT student.email FROM `student` 
INNER JOIN login
ON student.email = login.username
WHERE student.std_ID = '$student_ID'";

$std_email = mysqli_query($connect,$student_email);
$email = mysqli_fetch_assoc($std_email);
...
WHERE student.email = '$std_email' AND enroll.en_sec = teach.t_sec AND enroll.en_sec = schduling_lec.section";
...
INNER JOIN teacher
ON teach.t_ID = teacher.t_ID
WHERE student.email = '$std_email' AND enroll.en_sec = teach.t_sec AND enroll.en_sec = schduling_lec.section AND slot_time.study_day = 'MTh'
...
INNER JOIN teacher
ON teach.t_ID = teacher.t_ID
WHERE student.email = '$std_email' AND enroll.en_sec = teach.t_sec AND enroll.en_sec = schduling_lec.section AND slot_time.study_day = 'TuF'
ORDER BY slot_time.comp_slot ASC";
...
<div class="container-fluid" style = "background-color: #eee; height:67px;">
    <div class="row">
        <div class="col-sm">
            ...
            <div style = "margin:"><h5>ระบบงานทะเบียนศึกษา</h5>
    ...
    <div class="row" style = "top: 15%; position: absolute;">
        ...
        <div class="col-md-auto" style =  "text-align: center; margin-left: 15;">
            <ul class="nav nav-tabs" style="font-size:22px; font-weight:bold;">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="http://127.0.0.1/RegWeb/TeacherInfo.php">นักศึกษาในการดูเเล</a>
                </li>
                <li class="nav-item">
            ...
            <h1 style = "text-align: left; font-size:25px; margin-top: 15;">ตารางเรียน</h1>
            <style>
                .mix, .mix td {
                    ...
                    <td colspan = "11">
                        <div class="container" style = "margin-left: -13 ">
                            ...
                            <?php $pixel = 868; $subject_TuF = array(); //เริ่มต้นที่ 7 โมง 868/7 = 124px
                                ...
                                $pixel += $Study_time;
                                ?>
                                <div class="col" style = "border-left: 1px solid #999; border-right: 1px solid #999; width: <?=$Study_time ?>px; height: 120px; text-align: center; padding:15; background-color: FF33E0"><?=$TuF['coursename']?><br><?=$TuF['course_ID'],' (',$TuF['section'],')'?><br><?=$TuF['lec_room'],', '?><?=$TuF['study_start'],'-',$TuF['study_finish']?><br></div>
                                <?php } ?>
                            </div>
                        </div>
                    </td>
            ...
            <div style = "text-align: left; margin-bottom: 150;">
                <a class="nav-link active" href="http://127.0.0.1/RegWeb/TeacherInfo.php">ย้อนกลับ</a>
            </div>
        </div>
    </div>
</div>
